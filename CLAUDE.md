...
2. **Plan.** Act as a senior architect turning the spec changes into an implementation plan. Use a **master-checklist-with-sub-plans** structure: `plans/<feature>/00-master.md` (the checklist) plus `plans/<feature>/NN-*.md` sub-plans, broken into small, independently testable chunks. These chunks describe modular, self-contained units of work — **implement them inline yourself; do not spawn subagents unless the user explicitly asks.** Ask lots of questions about how product intent should translate to technical implementation. If planning uncovers details that warrant product changes, **return to stage 1** and update the specs first. **Then write the plan files, post a summary, and STOP** — wait for the user's explicit "proceed" before implementing. Plans are ephemeral (see stage 4).
...
4. **User acceptance.** The user tests the feature and either iterates or accepts. **On acceptance: delete the feature's `plans/<feature>/` directory and commit the feature.** Do not commit before acceptance.
