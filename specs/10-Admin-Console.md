...
- **One unified auth control.** The admin area uses the **same** header auth control as the public site rather than a separate, differently-styled "Sign out" button. Sign-in state and the logout action look and behave identically everywhere in the app. The control is **context-aware for its navigation affordance**: on the **public** site it shows **"Admin panel"** (into `/admin`); inside the **admin** area — where "Admin panel" would be a no-op link to the page you are already on — it instead shows **"Return to main view"**, which navigates the director back to the public default view (`/`, the root all visitors land on). "Logout" is unchanged in both contexts.
...
- From any public page, a signed-out visitor sees an **"Admin login"** button top-right; clicking it and signing in with an allowlisted Google account lands directly in `/admin`, after which every **public** page shows **"Admin panel"** + **"Logout"** instead; inside the **admin** area the same control reads **"Return to main view"** + **"Logout"**, and "Return to main view" navigates to the public default view (`/`). A non-allowlisted Google account is denied at the allowlist gate (standard AccessDenied), and **"Logout"** returns to the public site as a signed-out visitor.
