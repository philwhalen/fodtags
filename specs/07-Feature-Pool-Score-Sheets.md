...
- **Tie-break tag numbers are shown** (the Tag # column — each holder's **current tag**, i.e. the Overall tie-break value [Spec 02 §2.10](./02-Domain-Model-and-Scoring.md#210-tag-numbers--nightly-reassignment)), and a row whose Championship rank was resolved by tag number rather than a strict points difference is marked, consistent with the leaderboard's tie indicator ([Spec 04 §4.4](./04-Feature-Leaderboards.md#44-states)). A League-Night detail line whose **finish** was tie-broken uses that night's **tag-in** ([Spec 02 §2.6](./02-Domain-Model-and-Scoring.md#26-tie-breakers)), not the current tag.
